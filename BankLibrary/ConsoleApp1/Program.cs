...
namespace ConsoleApp1
{
    class Program
    {
        ...
        {
            Bank<Account> bank = new Bank<Account>("ПриватБанк");//Обобщенный класс приватбанка
            bool alive = true;
            while (alive)
            {
    ...
    private static void OpenAccount(Bank<Account> bank)
        ...
        private static void OpenAccountHandler(object sender,AccounEventArgs e)
        {
            Console.WriteLine(e.Message);
        }
        //обработчик событий добавления на счет денег
        private static void AddSumHandler(object sender,AccounEventArgs e)
        {
            Console.WriteLine(e.Message);
        }

        //обработчик вывода средств со счета
         private static void WithdrawSumHandler(object sender,AccounEventArgs e)
        {
            Console.WriteLine(e.Message);
            if(e.Sum>0)
                Console.WriteLine("Идем тратить деньги!");
        }
        private static void CloseAccountHandler(object sender,AccounEventArgs e)
        {
            Console.WriteLine(e.Message);
        }
